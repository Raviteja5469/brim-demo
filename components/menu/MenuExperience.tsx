  ...
  return (
    <div className="mx-auto max-w-7xl px-5 pb-20 sm:px-8 sm:pb-28">
      <div className="space-y-16">
        {DISPLAY_MENU.map((category, categoryIndex) => (
          <section key={category.id} id={`cat-${category.id}`} className="scroll-mt-28">
            ...
            <div className="grid gap-4 md:grid-cols-2 xl:grid-cols-3">{category.items.map((item, itemIndex) => <MenuItemCard key={item.slug} item={item} preload={categoryIndex === 0 && itemIndex < 3} />)}</div>
          </section>
        ))}
      </div>
    </div>
