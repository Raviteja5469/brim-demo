...
export function MenuItemCard({ item }: { item: MenuItem }) {
  ...
  const content = (
    <>
      <div className="relative overflow-hidden rounded-xl bg-[#f3f3f1]">
        {image ? <Image src={asset(image)} alt={item.name} fill sizes="176px" className="object-contain p-2 transition-transform duration-500 group-hover:scale-105" /> : <span className="grid h-full place-items-center font-display text-6xl text-ink/20">BRIM</span>}
  ...
  return (
    <article className="group overflow-hidden rounded-2xl bg-white p-3 shadow-sm ring-1 ring-ink/10 transition-transform duration-300 hover:-translate-y-1 hover:shadow-lg">
      {item.detailLink === false ? <div className="grid min-h-44 grid-cols-[10rem_minmax(0,1fr)] gap-3 sm:grid-cols-[11rem_minmax(0,1fr)]">{content}</div> : <Link href={`/menu/${item.slug}`} className="grid min-h-44 grid-cols-[10rem_minmax(0,1fr)] gap-3 sm:grid-cols-[11rem_minmax(0,1fr)]">{content}</Link>}
    </article>
  );
}
