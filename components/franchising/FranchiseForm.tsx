...
export function FranchiseForm() {
  ...
  if (sent) {
    return (
      <div className="rounded-3xl bg-ink p-8 text-paper ring-1 ring-white/10 sm:p-10" role="status">
        <span className="grid h-12 w-12 place-items-center rounded-full bg-paper font-bold text-ink">✓</span>
        <h3 className="mt-6 font-display text-4xl uppercase">Your pack is unlocked</h3>
        <p className="mt-3 max-w-lg text-sm leading-relaxed text-paper/55">Your email app has been opened with the enquiry ready to send. Send that message so the franchise team receives your lead alert.</p>
        <a href={asset("/BRIM-Franchise-Introduction.html")} download className="mt-7 inline-flex rounded-full bg-paper px-6 py-3 text-sm font-semibold uppercase tracking-wide text-ink transition-transform duration-300 hover:scale-[1.03]">Download franchise pack</a>
      </div>
    );
  }

  return (
    <form onSubmit={submit} noValidate className="rounded-3xl bg-ink p-6 text-paper ring-1 ring-white/10 sm:p-8">
      <ol className="grid grid-cols-3 gap-2" aria-label="Enquiry progress">
        {STEPS.map((label, index) => (
          <li key={label} aria-current={step === index ? "step" : undefined}>
            <div className={`h-1.5 rounded-full transition-colors duration-300 ${index <= step ? "bg-paper" : "bg-white/15"}`} />
            <span className={`mt-2 block text-xs font-bold uppercase tracking-wider ${index === step ? "text-paper" : "text-paper/35"}`}>{index + 1}. {label}</span>
          </li>
        ))}
      </ol>
      ...
      <div className="mt-8">
        ...
        {step === 1 && (
          <fieldset>
            ...
            <p className="mt-5 text-xs text-paper/40">All cost figures are indicative, confirmed after site inspection.</p>
          </fieldset>
        )}
        ...
        {step === 2 && (
          <fieldset>
            ...
            <label className="mt-6 flex cursor-pointer items-start gap-3 text-xs leading-relaxed text-paper/55">
              <input type="checkbox" checked={values.consent} onChange={(event) => set("consent", event.target.checked)} className="mt-0.5 h-4 w-4 accent-paper" />
      ...
      <div className="mt-8 flex items-center justify-between gap-3 border-t border-white/10 pt-6">
        {step > 0 ? <button type="button" onClick={() => setStep((current) => current - 1)} className="rounded-full border border-white/20 px-5 py-3 text-sm font-semibold uppercase tracking-wide text-paper transition-colors duration-300 hover:bg-white/10">Back</button> : <span />}
        {step < 2
          ? <button type="button" onClick={nextStep} className="rounded-full bg-paper px-6 py-3 text-sm font-semibold uppercase tracking-wide text-ink transition-transform duration-300 hover:scale-[1.03]">Continue</button>
          : <button type="submit" className="rounded-full bg-paper px-6 py-3 text-sm font-semibold uppercase tracking-wide text-ink transition-transform duration-300 hover:scale-[1.03]">Submit & unlock pack</button>}
      </div>
    </form>
  );
}
...
function Input({ id, label, value, onChange, error, type = "text", autoComplete }: { id: string; label: string; value: string; onChange: (value: string) => void; error?: string; type?: string; autoComplete: string }) {
  return (
    <label htmlFor={id} className="block">
      <span className="mb-2 block text-xs font-bold uppercase tracking-wider text-paper/55">{label}</span>
      <input id={id} name={id} type={type} value={value} onChange={(event) => onChange(event.target.value)} autoComplete={autoComplete} aria-invalid={!!error} aria-describedby={error ? `${id}-error` : undefined} className={`w-full rounded-xl border bg-white/5 px-4 py-3 text-paper outline-none transition-colors duration-300 ${error ? "border-red-500" : "border-white/15 focus:border-white/50"}`} />
      {error && <span id={`${id}-error`} className="mt-1.5 block text-xs text-red-400">{error}</span>}
    </label>
  );
}
...
function Select({ id, label, value, onChange, error, options }: { id: string; label: string; value: string; onChange: (value: string) => void; error?: string; options: string[] }) {
  return (
    <label htmlFor={id} className="block">
      <span className="mb-2 block text-xs font-bold uppercase tracking-wider text-paper/55">{label}</span>
      <select id={id} name={id} value={value} onChange={(event) => onChange(event.target.value)} aria-invalid={!!error} aria-describedby={error ? `${id}-error` : undefined} className={`w-full rounded-xl border bg-ink-soft px-4 py-3 text-paper outline-none transition-colors duration-300 ${error ? "border-red-500" : "border-white/15 focus:border-white/50"}`}>
